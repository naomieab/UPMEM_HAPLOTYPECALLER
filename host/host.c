...
//activate dpus
uint32_t dpu_inactive[NR_REGIONS];
...
int main(int argc, char* argv[]) {
	...
	int dpu_iterations = (int)(ceil((double)TOTAL_REGIONS / nr_dpus)) + 1;
	...
	for (int iteration = 1; iteration < dpu_iterations; iteration++) {
		printf("Starting iteration: %d\n", iteration);
		if (iteration == (int)ceil((double)TOTAL_REGIONS / nr_dpus)) {
