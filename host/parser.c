...
FILE* read_data(FILE* file, int nr_dpus) {
	...
	while (current_dpu < nr_dpus && fgets(buffer, BUFFER_SIZE, file) != 0 && current_region < NR_REGIONS) {
		...
		int nr_haplotypes_current_region = atoi(buffer);
		...
		int nr_reads_current_region = atoi(fgets(buffer, BUFFER_SIZE, file));
        ...
        int region_complexity = sum_haplotypes_lengths*sum_read_lengths;
